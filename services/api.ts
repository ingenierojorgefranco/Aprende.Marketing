...
export const api = {
  ...
  updateArticle: async (id: string, article: Omit<Article, 'id' | 'createdAt'>): Promise<void> => {
      try {
          await fetchWithFallback(`/articles/${id}`, {
           ...
           if (isOfflineMode) {
                mockArticles = mockArticles.map(a => a.id === id ? { ...article, id, createdAt: a.createdAt } : a);
           }
           throw e;
      }
  },
