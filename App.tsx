...
// ======================================================
//  VISTA PÚBLICA DE LANDING (subdominio + slug)
//  URL: https://client.aprende.marketing/lp/:slug
//  Llama al backend: GET /api/public/pages/:slug
// ======================================================
...
const PublicLandingView: React.FC = () => {
  ...
  const palette = content.palette || 'default';
  const brandName = content.brandName || page.name;
  const testimonials = content.testimonials || [];
  const benefits = content.benefits?.items || [];
  const testimonialTitle = content.testimonialTitle || 'Testimonios';
  const logoSvg = content.logoSvg as string | undefined;
  ...
  return (
    <div className="min-h-screen bg-black text-white">
          ...
          <header className="flex items-center justify-between mb-10">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                {page.niche && <p className="text-xs text-gray-400">{page.niche}</p>}
              </div>
            </div>
            {hero?.spotsLeft && (
          ...
          <div className="grid md:grid-cols-2 gap-10 items-center">
            <div>
              ...
              <h2
                ...
                dangerouslySetInnerHTML={{ __html: hero.headline || page.name }}
              />
              ...
              <div className="flex flex-wrap items-center gap-4">
                <a
                  href={content.destination?.type === 'external' ? content.destination.url : '#lead-form'}
              ...
              <div className="mt-6 flex gap-6 text-xs text-gray-500">
                ...
                {page.conversions != null && (
                  <span>
                    ✅ Registros: <b className="text-gray-200">{page.conversions}</b>
                  </span>
                )}
              </div>
            </div>
              ...
              <div className="aspect-[4/3] rounded-2xl overflow-hidden border border-gray-800 bg-gray-900/60 shadow-2xl">
                {hero.heroImage ? (
                  ...
                  <div className="w-full h-full flex items-center justify-center text-gray-500 text-sm px-6 text-center">
                    Vista previa de la landing generada. Configura una imagen principal en el
                    editor.
                  </div>
                )}
              </div>
      ...
      <footer className="border-t border-gray-900 bg-black py-6 text-center text-xs text-gray-500">
        <p>© {new Date().getFullYear()} {brandName || 'Plataforma de Venta'}. Todos los derechos reservados.</p>
      </footer>
    </div>
  );
};
