...
export const SubscriptionSuccessPage: React.FC<SubscriptionSuccessPageProps> = ({ user, onLogin }) => {
  ...
  const paramEmail = queryParams.get('email') || queryParams.get('buyer_email') || '';
  const paramName = queryParams.get('name') || queryParams.get('buyer_name') || '';
  ...
  useEffect(() => {
    const loadDetails = async () => {
      try {
        ...
        if (paramName) queryObj.name = paramName;
  ...
  const planName = data?.plan?.name || (paramPlan ? paramPlan.toUpperCase() : 'Plan Pro All-Access');
  ...
  const amount = data?.purchase?.amount || paramPrice || (data?.plan?.priceMonthly ? data.plan.priceMonthly : '79');
    ...
    <div className="min-h-screen bg-[#07090E] text-slate-100 flex flex-col selection:bg-orange-500 selection:text-white">
      ...
      <main className="relative z-10 flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 py-10 lg:py-16">
        ...
        <div className="text-center max-w-3xl mx-auto mb-12 lg:mb-16">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-orange-500/15 border border-orange-500/30 text-orange-400 text-xs sm:text-sm font-bold tracking-wide uppercase mb-6 shadow-inner animate-pulse">
            <Sparkles className="w-4 h-4 text-orange-400" /> ¡Enhorabuena! Compra Exitosa en Hotmart
          </div>

          <h1 className="text-3xl sm:text-5xl lg:text-6xl font-black text-white tracking-tight leading-tight mb-6">
            ¡Tu Suscripción está <span className="text-transparent bg-clip-text bg-gradient-to-r from-orange-400 via-amber-300 to-emerald-400">100% Activa</span>!
          </h1>
            ...
            <div className="bg-gradient-to-b from-white/[0.08] to-white/[0.02] border border-white/10 rounded-3xl p-6 sm:p-8 backdrop-blur-xl shadow-2xl relative overflow-hidden">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-white/10">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <span className="text-xs uppercase font-bold tracking-wider text-emerald-400">Estado del Pedido</span>
                    <h3 className="text-xl font-bold text-white">Pago Aprobado</h3>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 py-6 border-b border-white/10 text-sm">
                ...
                <div>
                  ...
                  <span className="font-bold text-white text-base">
                    {amount ? `${amount} ${currency}` : 'Confirmado'}
                  </span>
                </div>
                ...
                <div>
                  <span className="text-gray-400 block text-xs font-semibold uppercase tracking-wider mb-1">Email Registrado</span>
                    ...
                    <form onSubmit={handleActivateSubmit} className="space-y-4">
                      ...
                      >
                        {formLoading ? (
                          ...
                          <>
                            <span>Activar y Entrar al Dashboard</span>
