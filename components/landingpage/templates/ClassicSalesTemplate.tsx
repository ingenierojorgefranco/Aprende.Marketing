...
export const ClassicSalesTemplate: React.FC<TemplateProps> = ({ content, ds, isMobilePreview, pageId, basePath, hasBlogArticles }) => {
        ...
        <div className="w-full max-w-[75em] mx-auto px-6">
            <div className={`grid gap-12 items-center ${isMobilePreview ? 'grid-cols-1' : 'lg:grid-cols-2'}`}>
                ...
                <div id="contenedor-texto-intro" className="relative z-10">
                    <span className={`inline-block py-1 px-3 rounded-full text-xs font-bold uppercase tracking-wider mb-6 ${ds.hero.badgeBg} ${ds.hero.badgeText}`}>Descubre Más</span>
                    <h2 className={`text-3xl md:text-4xl font-black mb-8 leading-tight ${ds.hero.titleColor}`}>{content.intro.title}</h2>
                    <div className={`space-y-6 text-lg leading-relaxed ${ds.hero.subtitleColor}`}>{renderRichText(content.intro.description)}</div>
    ...
    <section id="seccion-beneficios" className={`py-24 ${ds.features.sectionBg}`}>
        <div className="w-full max-w-[75em] mx-auto px-6">
            <div className="text-center mb-16">
                ...
                <p className={`text-lg max-w-2xl mx-auto mt-4 leading-relaxed ${ds.features.descColor}`}>{content.benefits.subtitle || "Recibe el arsenal completo de recursos que han llevado a nuestras alumnas a facturar desde su primer mes."}</p>
  ...
  const StepsSection = () => (
    <section id="seccion-pasos" className={`py-24 relative ${ds.bg}`}>
        <div className="w-full max-w-[75em] mx-auto px-6">
            <div className="text-center mb-16">
                <h2 className={`text-3xl md:text-4xl font-bold mb-4 ${ds.hero.titleColor}`}>Acceder a tu Transformación es Muy Fácil</h2>
                <p className={`text-lg max-w-2xl mx-auto ${ds.hero.subtitleColor}`}>En solo 3 simples pasos estarás dentro de la clase que puede cambiar tu carrera.</p>
            </div>
            <div className={`relative grid gap-8 ${isMobilePreview ? 'grid-cols-1' : 'md:grid-cols-3'}`}>
                 {[
                    ...
                    <div key={i} className={`relative z-10 flex flex-col items-center text-center p-8 rounded-2xl transition hover:-translate-y-2 border ${ds.features.cardBg} ${ds.features.cardBorder} ${ds.features.cardShadow}`}>
                         <div className={`w-16 h-16 rounded-full flex items-center justify-center text-2xl font-bold mb-6 ${ds.features.iconContainer}`}>{step.num}</div>
                         <h3 className={`text-xl font-bold mb-3 ${ds.features.titleColor}`}>{step.title}</h3>
                         <p className={`text-sm leading-relaxed ${ds.features.descColor}`}>{step.text}</p>
                    </div>
                 ))}
            </div>
        </div>
         ...
         <div className="w-full max-w-[75em] mx-auto px-6 relative z-10">
            <div className={`flex flex-col items-center gap-12 ${isMobilePreview ? '' : 'md:flex-row md:gap-20'}`}>
                <div className="relative group shrink-0">
                     ...
                     <div className={`absolute bottom-4 right-4 z-20 backdrop-blur-md border p-3 rounded-2xl shadow-lg flex items-center gap-2 ${ds.instructor.badgeBg} ${ds.instructor.badgeBorder}`}>
                ...
                {(content.testimonials || []).map((t, i) => (
                    <div key={i} className={`p-6 rounded-2xl flex flex-col gap-4 transition hover:-translate-y-1 backdrop-blur-sm border ${ds.testimonials.cardBg} ${ds.testimonials.cardBorder} ${ds.testimonials.cardShadow}`}>
                        ...
                        <div>
                            ...
                            <div className={`flex mt-3 gap-1 ${ds.testimonials.starColor}`}>{[...Array(5)].map((_, starI) => <Star key={starI} className="w-4 h-4" fill={starI < t.rating ? "currentColor" : "none"} stroke="currentColor"/>)}</div>
                        </div>
                    </div>
                ))}
            </div>
  ...
  const FinalCTASection = () => (
    <section id="seccion-cta-final" className={`py-24 relative overflow-hidden ${ds.cta.sectionBg}`}>
        ...
        <div className="w-full max-w-[75em] mx-auto px-6 text-center relative z-10">
            ...
            <p className={`text-lg mb-10 max-w-2xl mx-auto ${ds.hero.subtitleColor.includes('white') ? 'text-gray-200' : 'text-gray-700'}`}>No dejes pasar esta oportunidad. El acceso a la certificación y los bonos exclusivos termina pronto.</p>
  ...
  return (
    <div id="layout-ventas-clasica" className={`min-h-screen font-sans ${ds.selectionColor} ${ds.bg} scroll-smooth`}>
        ...
        <header id="cabecera-hero" className={`relative pb-12 overflow-hidden ${ds.hero.bgGradient} ${isMobilePreview ? 'pt-28' : 'pt-24 lg:pt-48 lg:pb-32'}`}>
