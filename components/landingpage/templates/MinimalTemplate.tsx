    ...
    <div id="layout-minimalista" className={`min-h-screen font-sans flex flex-col ${ds.bg}`}>
        <div className="flex-1 flex flex-col justify-center py-12 relative overflow-hidden">
             ...
             <div className="w-full max-w-lg mx-auto px-6 relative z-10 text-center">
                  <div className={`w-16 h-16 mx-auto mb-8 rounded-2xl flex items-center justify-center shadow-lg ${ds.nav.logoBg}`}>
                  ...
                  <h1 className={`text-3xl md:text-4xl font-bold mb-4 ${ds.hero.titleColor}`}>
                      {content.hero.headline.replace(/<\/?[^>]+(>|$)/g, "")}
                  </h1>
                  <p className={`mb-8 leading-relaxed ${ds.hero.subtitleColor}`}>
                      {content.hero.subheadline}
                  </p>

                  <div className={`p-1 rounded-2xl shadow-xl border ${ds.features.cardBg} ${ds.features.cardBorder}`}>
                      <SmartCTA content={content} ds={ds} isMobilePreview={isMobilePreview} fullWidth={true} />
                  </div>

                  <p className={`mt-8 text-xs ${ds.footer.copyrightColor}`}>
