...
import { Project, User, WhatsAppLaunchMessage } from '../../../types';
...
export const WhatsAppLaunchWizard: React.FC = () => {
    ...
    const [messages, setMessages] = useState<WhatsAppLaunchMessage[]>([]);
    ...
    const handleProjectSelect = async (project: Project) => {
        ...
        setLoading(true);
        try {
            const seqInfo = await api.createWhatsAppLaunchSequence(project.id, `Lanzamiento: ${project.name}`);
            const msgData = await api.getWhatsAppLaunchMessages(seqInfo.id);
            setMessages(msgData);
            setStep(1);
        } catch (e) {
            console.error(e);
            alert("Error al inicializar la secuencia");
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleUpdateMessage = async (index: number, field: string, value: any) => {
        ...
        try {
            await api.updateWhatsAppLaunchMessage(newMessages[index].id, { [field]: value });
    ...
    const handleGenerate = async () => {
        setGenerating(true);
        try {
            // Simulamos la lógica de generación que normalmente vendría de generateFullStrategy
            // Pero aquí lo hacemos específico para este momento si está vacío
    ...
    const handleCopy = () => {
        navigator.clipboard.writeText(messages[activeMsgIdx].content);
    ...
    return (
        <div className="max-w-[1400px] mx-auto bg-gray-900 rounded-[3rem] shadow-2xl border border-white/5 overflow-hidden min-h-[600px] flex flex-col relative animate-in fade-in duration-500">
            ...
            <div className="bg-emerald-500/10 p-8 text-center border-b border-emerald-500/10 shrink-0 relative">
                {step > 0 && (
                    <button 
                        onClick={() => { setStep(0); setSelectedProject(null); }}
                        className="absolute left-8 top-1/2 -translate-y-1/2 flex items-center gap-2 px-4 py-2 bg-gray-800/50 hover:bg-gray-800 text-gray-300 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all border border-white/5"
                    >
                ...
                <h2 className="text-2xl font-bold text-white uppercase tracking-wider">Gestor de Mensajes de Lanzamiento</h2>
            ...
            <div className="p-8 md:p-12 flex-1 overflow-y-auto">
                    ...
                    <div className="w-full grid grid-cols-1 md:grid-cols-2 gap-10 max-w-6xl mx-auto py-10">
                        {projects.map((project) => (
                            <div 
                                ...
                                className="p-10 bg-[#0B0B0B] border border-white/5 rounded-[3rem] hover:border-emerald-500/50 hover:bg-emerald-500/5 transition-all text-left group flex flex-col shadow-2xl cursor-pointer"
                            >
                ...
                {step === 1 && (
                    <div className="grid lg:grid-cols-12 gap-10 h-full">
                        ...
                        {/* EDITOR DE MENSAJE */}
                        <div className="lg:col-span-8 bg-[#0B0B0B] border border-gray-800 rounded-[3rem] p-8 md:p-10 flex flex-col shadow-2xl">
                            {saveIndicator && (
                            ...
                            <div className="flex justify-between items-center mb-8">
                                <div>
                                    <h3 className="text-2xl font-black text-white">{messages[activeMsgIdx]?.name}</h3>
                                    <p className="text-emerald-400 text-xs font-bold uppercase tracking-widest mt-1">{messages[activeMsgIdx]?.momentText}</p>
                                </div>
                                <div className="flex gap-3">
                                    ...
                                    >
                                        <Wand2 className="w-4 h-4" /> {messages[activeMsgIdx]?.isGenerated ? 'Regenerar con IA' : 'Generar con IA'}
                                    </button>
                                </div>
                            </div>
                            ...
                            <div className="bg-emerald-500/5 border border-emerald-500/10 p-6 rounded-2xl mb-8 flex gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-gray-400 text-sm leading-relaxed">{messages[activeMsgIdx]?.objective}</p>
                                </div>
                            </div>
                            ...
                            <div className="flex-1 flex flex-col">
                                ...
                                <textarea 
                                    value={messages[activeMsgIdx]?.content}
                                ...
                                <div className="flex justify-between items-center mt-6">
                                    ...
                                    <button 
                                        onClick={handleCopy}
                                        disabled={!messages[activeMsgIdx]?.content}
