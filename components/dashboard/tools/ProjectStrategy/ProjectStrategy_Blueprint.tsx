...
import { Layers, ArrowRight, ArrowDown, PlayCircle, Clapperboard, Globe, CheckCircle2, Users, MessageCircle, FileText, MonitorPlay, ShoppingCart, Sparkles, Zap } from 'lucide-react';
    ...
    return (
        <div id={`psd-flow-card-${step}-container`} className="group relative flex flex-col h-full">
            <div 
                ...
                className="bg-gray-900/80 backdrop-blur-sm border border-gray-800 p-7 rounded-[2rem] flex flex-col items-center text-center relative z-10 hover:border-primary/50 transition-all duration-500 hover:scale-[1.03] hover:shadow-[0_20px_50px_rgba(99,102,241,0.15)] h-full group-hover:z-50 cursor-help"
                onMouseMove={(e) => onHover(e, tooltipContent)}
                onMouseLeave={onLeave}
            >
                {/* Step Number Badge */}
                <div className="absolute -top-5 -right-5 w-14 h-14 bg-gradient-to-br from-primary to-indigo-600 border-4 border-black rounded-2xl flex items-center justify-center text-xl font-black text-white shadow-2xl z-20 group-hover:rotate-12 transition-transform">
                    {step}
                </div>

                {/* Animated Background Pulse */}
                <div className="absolute inset-0 bg-primary/5 opacity-0 group-hover:opacity-100 rounded-[2rem] transition-opacity duration-700 animate-pulse"></div>

                <div className="w-20 h-20 rounded-2xl bg-gray-800 flex items-center justify-center mb-6 group-hover:bg-primary/20 transition-colors shadow-inner relative">
                    <Icon className="w-10 h-10 text-gray-400 group-hover:text-primary transition-colors" />
                    {/* Visual data pulse */}
                    <div className="absolute -bottom-1 -right-1 w-4 h-4 bg-green-500 rounded-full border-2 border-black animate-ping opacity-0 group-hover:opacity-100"></div>
                </div>

                <h4 className="text-white font-black text-xl mb-1 tracking-tight">{title}</h4>
                <p className="text-[10px] text-primary font-black uppercase tracking-[0.2em] mb-4 bg-primary/10 px-3 py-1 rounded-full">{subtitle}</p>
                ...
                >
                    <PlayCircle className="w-5 h-5 text-primary" /> Explicación Táctica
                </button>
            </div>
            ...
            {!isLastItem && (
                <>
                    {!isLastInRow && (
                        <div id={`psd-flow-card-${step}-arrow-right`} className="hidden lg:flex absolute top-1/2 -right-12 w-12 items-center justify-center z-0 text-primary/30 animate-pulse">
                    ...
                    {isEndOfRowDesktop && (
                        <div id={`psd-flow-card-${step}-arrow-snake`} className="hidden lg:block absolute top-full right-1/2 w-[calc(200%+6rem)] h-32 pointer-events-none z-0">
                           <div className="absolute right-[-1px] top-0 h-16 w-0.5 bg-gradient-to-b from-primary/30 to-primary/10 animate-pulse"></div>
                           <div className="absolute right-0 top-16 left-0 h-0.5 bg-gradient-to-r from-primary/10 via-primary/30 to-primary/10"></div>
                           <div className="absolute left-[-1px] top-16 h-16 w-0.5 bg-gradient-to-b from-primary/10 to-primary/30 animate-pulse"></div>
                            <div className="absolute left-[-0.65rem] bottom-[-0.6rem] text-primary/40">
                                <ArrowDown className="w-6 h-6" />
                            </div>
                        </div>
                    )}
                    <div id={`psd-flow-card-${step}-arrow-down`} className="lg:hidden flex justify-center py-6 text-primary/30 animate-pulse">
        ...
        <div id="psd-blueprint-container" className="space-y-12">
            <div id="psd-blueprint-header-container" className="max-w-[70em] mx-auto text-left space-y-8 py-10">
                <div className="inline-flex items-center gap-3 px-5 py-2 rounded-full bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 text-sm font-black uppercase tracking-[0.2em] shadow-lg shadow-indigo-500/5">
                    <Zap className="w-5 h-5 fill-current" /> Plano de Adquisición
                </div>
                <h3 id="psd-blueprint-title" className="text-5xl md:text-6xl font-black text-white leading-tight tracking-tight max-w-4xl">
                    Tu sistema de ventas de <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-purple-500">Clase Mundial</span>
                </h3>
                <div className="grid md:grid-cols-2 gap-10 text-gray-300 text-[1.4rem] leading-[1.8] font-light">
                    <p className="border-l-4 border-indigo-500/30 pl-8 py-2">
            ...
            <div id="psd-blueprint-grid-wrapper" className="max-w-[95em] mx-auto rounded-[3.5rem] border border-gray-800 p-12 lg:p-20 relative overflow-hidden shadow-[0_40px_100px_rgba(0,0,0,0.5)]">
                ...
                <div className="absolute inset-0 bg-[#050505] z-0">
                     ...
                     <div className="absolute top-0 left-0 w-full h-full bg-gradient-to-b from-indigo-900/10 via-transparent to-transparent"></div>
                </div>
                ...
                {/* Animated Scan Line */}
                <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-transparent via-primary/40 to-transparent animate-scan-line pointer-events-none"></div>
            </div>
        </div>
    );
};