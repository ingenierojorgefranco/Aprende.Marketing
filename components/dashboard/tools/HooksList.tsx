                ...
                {hooks.length === 0 ? (
                    ...
                    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                        {hooks.map((hook) => (
                            <div 
                                key={hook.id} 
                                onClick={() => navigate(`/dashboard/projects/${hook.projectId}/strategy?section=hooks`)}
                                ...
                                <div className="p-8 flex-1 flex flex-col">
                                    <div className="flex justify-between items-start mb-6">
                                        <div className="bg-white/5 text-white text-[0.8em] px-3 py-1 rounded-full flex items-center gap-1.5 w-fit border border-white/5 font-black uppercase tracking-widest">
                                            ...
                                            {(hook as any).projectName || "Proyecto"}
                                        </div>
                                        <button 
                                            onClick={(e) => handleDelete(hook, e)}
                                            className="p-2 bg-red-900/20 hover:bg-red-500 rounded-xl text-red-500 hover:text-white transition-all shadow-lg"
                                        >
                                            <Trash2 className="w-4 h-4" />
                                        </button>
                                    </div>
                                    
                                    <h3 className="text-2xl font-black mb-3 line-clamp-1 group-hover:text-orange-400 transition-colors duration-300 text-white">{hook.title}</h3>
                                    
                                    <div className="bg-orange-500/5 rounded-2xl p-4 border border-orange-500/10 mb-6 flex-1">
                                        <div className="flex items-center gap-2 mb-2">
                                            <Brain className="w-4 h-4 text-orange-400" />
                                            <span className="text-[10px] font-black uppercase text-orange-300 tracking-widest">Estrategia</span>
                                        </div>
