...
export const WhatsAppLaunchManager: React.FC = () => {
    ...
    // --- Estado para el Wizard Integrado ---
    const [isWizardOpen, setIsWizardOpen] = useState(false);
    const [editingLaunchId, setEditingLaunchId] = useState<string | undefined>(undefined);
    ...
    const maxLaunches = projects.reduce((sum, p) => {
        if (p.limitsConfig?.maxWhatsAppLaunches) return sum + p.limitsConfig.maxWhatsAppLaunches;
    ...
    return (
        <div className="space-y-10 animate-in fade-in duration-500 pb-20">
            ...
            {/* SECCIÓN: MIS LANZAMIENTOS O WIZARD */}
            {isWizardOpen ? (
