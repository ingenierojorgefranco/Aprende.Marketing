    ...
    return (
      <div className="mb-3">
        <div
          ...
          className={`relative w-full flex items-center justify-between px-5 py-4 rounded-xl transition-all duration-200 cursor-pointer overflow-hidden border ${
            isActive 
        ...
        >
          <div className="flex items-center gap-4 relative z-10">
            <item.icon className={`w-6 h-6 shrink-0 ${isActive ? 'text-white' : 'text-[#B0B0B0]'}`} />
    ...
    <div className="h-screen overflow-hidden bg-[#030712] text-[#FFFFFF] flex font-sans">
      {(!showWelcomeVideo && !showWizard && !isProjectDetailRoute) && (
        <aside className={`fixed md:relative top-0 left-0 h-full w-64 md:w-[17rem] shrink-0 bg-[#030712] border-r border-slate-800/60 shadow-2xl z-40 transition-all duration-300 flex flex-col ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
          ...
          <nav className="flex-1 px-4 py-4 space-y-4 overflow-y-auto custom-scrollbar">{menuStructure.map(item => <NavItemRender key={item.id} item={item} />)}</nav>
                 ...
                 <div className="flex items-center gap-2.5 sm:gap-3">
                     ...
                     {!isLaunchRestricted && (
                         <button 
                             ...
                             title="Actualiza a Plan Pro"
                         >
                             <div className="flex items-center gap-2 sm:gap-2.5 relative z-10">
                                 <div className="w-6 h-6 sm:w-7 sm:h-7 rounded-lg bg-gradient-to-br from-yellow-400 via-amber-500 to-yellow-600 flex items-center justify-center shadow-md shadow-amber-950/40 shrink-0 group-hover:scale-105 transition-transform">
                                     <Crown className="w-3.5 h-3.5 text-black fill-black/20" />
                                 </div>
                                 <span className="text-[11px] sm:text-xs font-black tracking-wider uppercase text-transparent bg-clip-text bg-gradient-to-r from-yellow-200 via-amber-300 to-yellow-400 group-hover:from-white group-hover:to-yellow-200">
                                     ACTUALIZA A PRO
                                 </span>
                             </div>
                             <ChevronRight className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-yellow-400/80 group-hover:text-yellow-300 group-hover:translate-x-0.5 transition-all relative z-10 ml-0.5" />
                         </button>
                     )}
