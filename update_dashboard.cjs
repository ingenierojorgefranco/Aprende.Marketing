...
// 1. Add state
const statePattern = /const \[projects, setProjects\] = useState<Project\[\]>\(\[\]\);/;
content = content.replace(statePattern, "const [projects, setProjects] = useState<Project[]>([]);\n  const [academyCourses, setAcademyCourses] = useState<any[]>([]);");

// 2. Fetch data
const fetchPattern = /const \[summary, userProjects\] = await Promise\.all\(\[\s+api\.getAnalyticsSummary\(\),\s+api\.getProjects\(\)\s+\]\);\s+setProjects\(userProjects \|\| \[\]\);/;
const fetchReplacement = `const [summary, userProjects, courses] = await Promise.all([
            ...
            setAcademyCourses((courses || []).slice(0, 3));`; // Limit to 3 items like grid-cols-3
content = content.replace(fetchPattern, fetchReplacement);

// 3. Remove mock data
const mockPattern = /\/\/ Mock data for academy[\s\S]*?\];/;
...
// We should also change grid-cols-4 to grid-cols-3
...
// 5. Navigate to training when clicking ver todas
const verTodasPattern = /<button className="text-\[#FF5A1F\] text-xs font-bold flex items-center gap-1 hover:underline">/
const verTodasReplacement = `<button onClick={() => navigate('/dashboard/training')} className="text-[#FF5A1F] text-xs font-bold flex items-center gap-1 hover:underline">`
content = content.replace(verTodasPattern, verTodasReplacement);
